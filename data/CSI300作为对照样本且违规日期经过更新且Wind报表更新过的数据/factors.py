...
def industry_match(CSI300_factor, violation_factor):
    CSI300_factor_final = pd.DataFrame()
    violation_info = violation_factor[['violation_year', 'INDUSTRY_CSRC12_N']].drop_duplicates()
    print('各年份匹配行业的数量')
    for i in range(violation_info.shape[0]):
        tmp = CSI300_factor.loc[(CSI300_factor.violation_year==violation_info.iloc[i,]['violation_year'])&
                                (CSI300_factor.INDUSTRY_CSRC12_N==violation_info.iloc[i,]['INDUSTRY_CSRC12_N']),]
        tmp = tmp.sort_values(by='账面市值', ascending=False).iloc[:10,]
        CSI300_factor_final = CSI300_factor_final.append(tmp)
        print(violation_info.iloc[i,]['violation_year'], violation_info.iloc[i,]['INDUSTRY_CSRC12_N'], len(tmp))
    return CSI300_factor_final

def data_preparation() -> pd.DataFrame:
    os.chdir('./data/CSI300作为对照样本且违规日期经过更新且Wind报表更新过的数据')
    violation_bs = pd.read_excel('violation_bs.xlsx')
    # violation_bs = violation_bs.loc[(violation_bs.violation_year >= 2003)&(violation_bs.violation_year <= 2019), ]
    ...
    # violation_is = violation_is.loc[(violation_is.violation_year >= 2003)&(violation_is.violation_year <= 2019), ]
    ...
    # violation_cs = violation_cs.loc[(violation_cs.violation_year >= 2003)&(violation_cs.violation_year <= 2019), ]
    ...
    # violation_other = violation_other.loc[(violation_other.violation_year >= 2003) & (violation_other.violation_year <= 2019),]
    ...
    for symbol in violation_bs.groups:
        violation_factor = violation_factor.append(current_period_factor_calculation(violation_bs.get_group(symbol), violation_is.get_group(symbol), violation_cs.get_group(symbol), violation_other.get_group(symbol)), ignore_index=True)
    violation_factor = violation_factor.loc[~violation_factor.symbol.isin(['200992.SH', '000691.SZ', '600816.SH']),:].sort_values(by=['symbol','sheet_year']).reset_index(drop=True)
    violation_factor['Y'] = 1

    CSI300_bs_industry = pd.read_excel('CSI300_symbol_industry.xlsx')
    CSI300_bs_industry = CSI300_bs_industry.loc[~CSI300_bs_industry.symbol.isin(['金融、保险业', '金融业']),]
    CSI300_bs_industry = CSI300_bs_industry.loc[~CSI300_bs_industry.symbol.isin(violation_bs.symbol.unique()),]
    CSI300_bs = pd.read_excel('CSI300_bs.xlsx')
    CSI300_bs = CSI300_bs_industry.merge(CSI300_bs, how='left', on=['symbol', 'sheet_year'])
    CSI300_bs = CSI300_bs.sort_values(by=['symbol','sheet_year']).groupby(by=['symbol', 'violation_year'])
    CSI300_is = pd.read_excel('CSI300_is.xlsx')
    CSI300_is = CSI300_bs_industry.merge(CSI300_is, how='left', on=['symbol', 'sheet_year'])
    CSI300_is = CSI300_is.sort_values(by=['symbol','sheet_year']).groupby(by=['symbol', 'violation_year'])
    CSI300_cs = pd.read_excel('CSI300_cs.xlsx')
    CSI300_cs = CSI300_bs_industry.merge(CSI300_cs, how='left', on=['symbol', 'sheet_year'])
    CSI300_cs = CSI300_cs.sort_values(by=['symbol','sheet_year']).groupby(by=['symbol', 'violation_year'])
    CSI300_other = pd.read_excel('CSI300_other.xlsx')
    CSI300_other = CSI300_other.sort_values(by=['symbol', 'sheet_year']).groupby(by=['symbol', 'violation_year'])
    CSI300_factor = pd.DataFrame()
    for symbol in CSI300_bs.groups:
        CSI300_factor = CSI300_factor.append(current_period_factor_calculation(CSI300_bs.get_group(symbol), CSI300_is.get_group(symbol), CSI300_cs.get_group(symbol), CSI300_other.get_group(symbol)), ignore_index=True)
    CSI300_factor['Y'] = 0
    CSI300_factor = CSI300_factor.loc[(~CSI300_factor.isna()).all(axis=1),]
    CSI300_factor_final = industry_match(CSI300_factor, violation_factor)

    data = pd.concat([CSI300_factor_final, violation_factor], axis=0, ignore_index=True).drop('账面市值', axis=1)
    return data
